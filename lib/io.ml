(** Device callback records (port of [risc-io.h] / [io.rs]).
    ...
    idiomatic OCaml equivalent is a record of closures, each capturing its own
    device's mutable state. The core ([Risc.t]) holds each device as an [option]
    and invokes it synchronously from inside the CPU step ([load_io]/[store_io]).
    Every value passed or returned is a machine word in [u32] range. *)
