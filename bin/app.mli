...
(** Map a key event to a frontend action ([sdl-main.c]'s hotkey table: Alt+F4 quit,
    F12 / Ctrl+Shift+Del reset, F11 / Alt+Enter / Cmd+Shift+F fullscreen, and left Alt
    as a fake middle mouse button on both press and release). Pure; exposed for
    testing. *)
val map_action
  :  down:bool
  -> keycode:int
  -> kmod:int
  -> [ `Quit | `Reset | `Toggle_fullscreen | `Fake_mouse2 | `Oberon_input ]
