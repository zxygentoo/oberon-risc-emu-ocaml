...
let () =
  ...
  eqs "encode_get" (Io.encode_request (Get { name = "M.Mod" })) "\xA5\x02\x05M.Mod";
  eqs
    "encode_put"
    (Io.encode_request (Put { name = "X"; data = "hi" }))
    "\xA5\x01\x01X\x02\x00\x00\x00hi";
  eqs
    "encode_call"
    (Io.encode_request (Call { cmd = "A.B"; par = "p" }))
    "\xA5\x03\x03A.B\x01\x00\x00\x00p";
  eqs
    "encode_edit"
    (Io.encode_request (Edit { name = "X"; old = "ab"; new_ = "c" }))
    "\xA5\x04\x01X\x02\x00\x00\x00ab\x01\x00\x00\x00c";
  eqs
    "encode_edit_empty_new"
    (Io.encode_request (Edit { name = "X"; old = "a"; new_ = "" }))
  ...
  expect_bad_name "empty_name_rejected" (fun () ->
    Io.encode_request (Get { name = "" }));
  expect_bad_name "long_name_rejected" (fun () ->
    Io.encode_request (Get { name = String.make 256 'x' }));
  check
    "name_at_255_accepted"
    (String.length (Io.encode_request (Get { name = String.make 255 'x' }))
     = 2 + 1 + 255);
  (* Response decoding. *)
  let r = read_with "\x5A\x00\x03\x00\x00\x00abc" in
  check "ok_with_payload_status" (r.status = Ok);
  eqs "ok_with_payload_bytes" r.payload "abc";
  check "ok_with_payload_ok" (ok r);
  ...
  for b = 0 to 255 do
    ...
    if status_byte r.status <> b then incr roundtrip_failures
  done;
  eq "status_byte_roundtrips_0_255" !roundtrip_failures 0;
  let r = read_with "\x5A\x2A\x00\x00\x00\x00" in
  check "unknown_status_preserved" (r.status = Other 0x2A);
  (* The device-side test codec inverts the production one. *)
  check
    "parse_inverts_get"
    (Io.For_tests.parse_request (Io.encode_request (Get { name = "M.Mod" }))
     = Get { name = "M.Mod" });
  check
    "parse_inverts_put"
    (Io.For_tests.parse_request (Io.encode_request (Put { name = "X"; data = "hi" }))
     = Put { name = "X"; data = "hi" });
  check
    "parse_inverts_call"
    (Io.For_tests.parse_request (Io.encode_request (Call { cmd = "A.B"; par = "p" }))
     = Call { cmd = "A.B"; par = "p" });
  check
    "parse_inverts_edit"
    (Io.For_tests.parse_request
       (Io.encode_request (Edit { name = "X"; old = "ab"; new_ = "" }))
     = Edit { name = "X"; old = "ab"; new_ = "" });
  (* ... and read_response inverts encode_response. *)
  let r =
    read_with
      (Io.For_tests.encode_response
         { status = Not_unique; payload = "\x02\x00\x00\x00" })
  in
  check "decode_inverts_encode_status" (r.status = Not_unique);
  eqs "decode_inverts_encode_payload" r.payload "\x02\x00\x00\x00";
  eq "not_unique_count" (not_unique_count r) 2;
  eq
    "not_unique_count_short_payload"
    (not_unique_count { status = Not_unique; payload = "" })
    0;
  (* Retry policy: only transport desyncs are re-sent, within the budget;
     everything else fails fast. *)
  let f, calls = flaky 0 timeout in
  check "first_try_ok" (ok (Io.with_retries ~retries:2 f));
  eq "first_try_one_call" !calls 1;
  (* Two timeouts then success; 2 retries (3 attempts) covers it. *)
  let f, calls = flaky 2 timeout in
  check "recovers_within_budget" (ok (Io.with_retries ~retries:2 f));
  eq "recovers_three_calls" !calls 3;
  let f, calls = flaky 1 bad_sync in
  check "bad_sync_retried" (ok (Io.with_retries ~retries:2 f));
  ...
  expect_error
    ...
    (function
      ...
      | _ -> false)
    (fun () -> Io.with_retries ~retries:2 f);
  ...
  expect_error
    ...
    (function
      | Error.Timeout _ -> true
      | _ -> false)
    (fun () -> Io.with_retries ~retries:0 f);
  ...
  expect_error
    ...
    (function
      | Error.Eof -> true
      | _ -> false)
    (fun () -> Io.with_retries ~retries:3 f);
