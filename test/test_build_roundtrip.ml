(* End-to-end round-trip for the image builder: extract the committed golden image into a
   ...
   OBERON_ROUNDTRIP=1 — keeping `dune runtest` fast. Run it with: OBERON_ROUNDTRIP=1 dune
   exec test/test_build_roundtrip.exe Byte-for-byte parity with the Rust build-po-image is
   checked out-of-band by diffing the two tools' output (see PORTING_HOST_TOOLS.md). *)
...
let () =
  ...
  then (
    ...
    exit 0);
  (* Resolve the golden image across both run contexts: `dune runtest` (cwd is the build
     test dir, dep is at ../DiskImage) and `dune exec` from the repo root. *)
  let dsk =
    List.find_opt
      Sys.file_exists
  ...
  match dsk with
  | None ->
    print_endline "test_build_roundtrip: skipped (no golden image)";
    exit 0
  | Some dsk ->
    let tmp =
      Filename.concat
