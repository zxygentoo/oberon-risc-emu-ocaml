...
let g_bool = G.bool
let g_op = G.oneof_list [ Mov; Lsl; Asr; Ror; And; Ann; Ior; Xor; Add; Sub; Mul; Div; Fad; Fsb; Fml; Fdv ]
let g_cond = G.oneof_list [ Mi; Eq; Cs; Vs; Ls; Lt; Le; True ]
let g_size = G.oneof_list [ W; B ]
let g_operand = G.oneof [ G.map (fun r -> Reg r) reg; G.map (fun i -> Imm i) (G.int_range 0 0xFFFF) ]
...
let g_target = G.oneof [ G.map (fun r -> To_reg r) reg; G.map (fun o -> To_off o) g_off24 ]
...
let e_reg q u v a b op ci =
  (q lsl 30) lor (u lsl 29) lor (v lsl 28) lor (a lsl 24) lor (b lsl 20) lor (op lsl 16) lor ci
;;

let e_mem u v a b off =
  0x8000_0000 lor (u lsl 29) lor (v lsl 28) lor (a lsl 24) lor (b lsl 20) lor (off land 0x000F_FFFF)
;;

let e_br_imm negate cond link off =
  0xE000_0000 lor (link lsl 28) lor (negate lsl 27) lor (cond lsl 24) lor (off land 0x00FF_FFFF)
;;
...
let tests =
  ...
  ; (* For any 32-bit word, decoding is stable through a re-encode: this is the
    ...
    Q.Test.make ~name:"codec: decode (encode (decode w)) = decode w" ~count:5000 u32 (fun w ->
  ...
  ; Q.Test.make
      ...
      (fun ((u, v), (a, b), op, operand) ->
        let q, ci = match operand with Reg c -> 0, c | Imm i -> 1, i land 0xFFFF in
        encode (Alu { op; u; v; a; b; operand }) = e_reg q (bit u) (bit v) a b (int_of_op op) ci)
  ; Q.Test.make
      ...
      (fun (size, (a, base), off, is_store) ->
        let v = match size with W -> 0 | B -> 1 in
        if is_store
  ...
  ; (* [decode]'s fields are exactly what the individual accessors read. *)
    Q.Test.make ~name:"decode fields agree with the accessors" ~count:5000 u32 (fun w ->
      ...
      | Load { size; a; base; off } ->
        ...
        && base = rb w
        && off = off20 w
        && (match size with W -> not (v w) | B -> v w)
